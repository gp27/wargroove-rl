...
class CustomPolicy(ActorCriticPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **kwargs):
        ...
        actions_n = ac_space.n

        with tf.variable_scope("model", reuse=reuse):

            obs, legal_actions = split_input(self.processed_obs, actions_n)

            extracted_features = resnet_extractor(obs, **kwargs)

            self._policy = policy_head(extracted_features, legal_actions, actions_n)
            self._value_fn, self.q_value = value_head(extracted_features, actions_n)
...
def value_head(y, actions_n):
    ...
    q = dense(y, actions_n, batch_norm = False, activation = 'tanh', name='q')
    return vf, q


def policy_head(y, legal_actions, actions_n):
    y = convolutional(y, 1, (1,1), batch_norm = False)
    y = Flatten()(y)
    y = dense(y,FEATURE_SIZE, batch_norm = False)

    policy = dense(y, actions_n, batch_norm = False, activation = None, name='pi')
